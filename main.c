/*  This file is part of "sshpass", a tool for batch running password ssh
 * authentication Copyright (C) 2006 Lingnu Open Source Consulting Ltd.
...
static void show_help() {
  printf("Usage: " PACKAGE_NAME " [-f|-d|-p|-e] [-hV] command parameters\n"
         ...
         "   -P prompt     Which string should sshpass search for to detect a "
         "password prompt\n"
         "   -r resend     Max number of times to send password. Useful for "
         "multi-hop.\n"
...
// Return argv offset on success, and a negative number on failure
...
#define VIRGIN_PWTYPE                                                          \
  ...
  while ((opt = getopt(argc, argv, "+f:d:p:P:he::r:Vv")) != -1 && error == -1) {
    switch (opt) {
    case 'f':
    ...
    case 'e':
      ...
      if (args.orig_password == NULL) {
        fprintf(stderr,
                "sshpass: -e option given but \"%s\" environment variable is "
                "not set.\n",
                optarg);
    ...
    case 'V':
      printf("%s\n"
             ...
             "This program is free software, and can be distributed under the "
             "terms of the GPL\n"
...
int runprogram(int argc, char *argv[]) {
  ...
  // We need to interrupt a select with a SIGCHLD. In order to do so, we need a
  // SIGCHLD handler
  ...
  /*
    ...
    We need to open the slavept inside the child process, after "setsid", so
    that it becomes the controlling TTY for the process. We do not, otherwise,
    need the file descriptor open. The original approach was to close the fd
    immediately after, as it is no longer needed.

    It turns out that (at least) the Linux kernel considers a master ptty fd
    that has no open slave fds to be unused, and causes "select" to return with
    "error on fd". The subsequent read would fail, causing us to go into an
    infinite loop. This is a bug in the kernel, as the fact that a master ptty
    fd has no slaves is not a permenant problem. As long as processes exist that
    have the slave end as their controlling TTYs, new slave fds can be created
    by opening /dev/tty, which is exactly what ssh is, in fact, doing.

    Our attempt at solving this problem, then, was to have the child process not
    close its end of the slave ptty fd. We do, essentially, leak this fd, but
    this was a small price to pay. This worked great up until openssh
    version 5.6.

    Openssh version 5.6 looks at all of its open file descriptors, and closes
    any that it does not know what they are for. While entirely within its
    prerogative, this breaks our fix, causing sshpass to either hang, or do the
    infinite loop again.

    Our solution is to keep the slave end open in both parent AND child, at
    least until the handshake is complete, at which point we no longer need to
    monitor the TTY anyways.
  */
  ...
  if (childpid == 0) {
    ...
    setsid();
    // This line makes the ptty our controlling tty. We do not otherwise need it
    // open
  ...
  do {
    if (!terminate) {
      ...
      int selret =
          pselect(masterpt + 1, &readfd, NULL, NULL, NULL, &sigmask_select);

      if (termsig != 0) {
        // Copying termsig isn't strictly necessary, as signals are masked at
        // this point.
        int signum = termsig;
        termsig = 0;
      ...
      if (selret > 0) {
        if (FD_ISSET(masterpt, &readfd)) {
          ...
          if ((ret = handleoutput(masterpt))) {
            ...
            // handleoutput returns positive error number in case of some error,
            // and a negative value if all that happened is that the slave end
            // of the pt is closed.
            if (ret > 0) {
              close(masterpt); // Signal ssh that it's controlling TTY is now
                               // closed
              close(slavept);
            }
...
int handleoutput(int fd) {
  ...
  static int prevmatch =
      0; // If the "password" prompt is repeated, we have the wrong password.
  static int state1, state2, state3;
  static int firsttime = 1;
  static const char *compare1 = PASSWORD_PROMPT; // Asking for a password
  static const char compare2[] =
      "The authenticity of host "; // Asks to authenticate host
  static const char compare3[] =
      "differs from the key for the IP address"; // Key changes
  // static const char compare3[]="WARNING: REMOTE HOST IDENTIFICATION HAS
  // CHANGED!"; // Warns about man in the middle attack The remote
  // identification changed error is sent to stderr, not the tty, so we do not
  // handle it. This is not a problem, as ssh exists immediately in such a case
  char buffer[256];
  int ret = 0;
  ...
  if (args.verbose && firsttime) {
    firsttime = 0;
    fprintf(stderr,
            "SSHPASS: searching for password prompt using match \"%s\"\n",
            compare1);
  }
  ...
  if (compare1[state1] == '\0') {
    if (prevmatch < args.max_resends) {
      ...
      if (args.verbose)
        fprintf(stderr,
                "SSHPASS: detected prompt too many times (%d). Terminating.\n",
                prevmatch);
...
int match(const char *reference, const char *buffer, ssize_t bufsize,
          int state) {
  // This is a highly simplisic implementation. It's good enough for matching
  // "Password: ", though.
  int i;
...
// Do nothing handler - makes sure the select will terminate if the signal
// arrives, though.
void sigchld_handler(int signum) {}

void term_handler(int signum) {
  // BUG: There is a potential race here if two signals arrive before the main
  // code had a chance to handle them. This seems low enough risk not to justify
  // the extra code to correctly handle this.
  termsig = signum;
}
...
void reliable_write(int fd, const void *data, size_t size) {
  ...
  if (result != size) {
    if (result < 0) {
      ...
      fprintf(stderr,
              "SSHPASS: Short write. Tried to write %lu, only wrote %ld\n",
              size, result);
    }
  }
}
